...
class WirelessAudioServer:
    ...
    def _validate_header_values(self, header: Dict[str, Any]) -> Tuple[bool, str, str]:
        ...
        # Allow enhanced headers (e.g., ESP32-P4 VAD extensions) to precede PCM data
        if actual_payload >= expected_payload:
            extra_bytes = actual_payload - expected_payload
            if extra_bytes > 0:
                # record extended metadata
                header['extended_header_bytes'] = extra_bytes
            return True, "Valid header", 'valid'
        
        # For undersized payloads, still treat within tolerance
        tolerance = 4  # Allow up to 4 bytes difference for alignment
        if expected_payload - actual_payload <= tolerance:
            header['extended_header_bytes'] = 0
            return True, "Valid header (undersized within tolerance)", 'valid'
    ...
    def _validate_and_extract_pcm_samples(self, data: bytes, header_info: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            extended_bytes = header_info.get('extended_header_bytes', 0)
            pcm_offset = self.UDP_HEADER_SIZE + max(0, extended_bytes)
            pcm_payload = data[pcm_offset:]
            ...
            if bits_per_sample == 16:
                try:
                    ...
                    pcm_samples = struct.unpack(pcm_format, pcm_payload[:expected_pcm_size])
                    
                    extra_payload = len(pcm_payload) - expected_pcm_size
                    max_amplitude = max(abs(sample) for sample in pcm_samples) if pcm_samples else 0
                    ...
                    return True, {
                        ...
                        'extended_header_bytes': header_info.get('extended_header_bytes', 0),
                        'extra_payload_bytes': extra_payload if extra_payload > 0 else 0
                    }
