...
class NetworkAudioSource:
    ...
    def send_tts_audio_to_devices(self, audio_file_path: str, text: str = "") -> bool:
        ...
        try:
            # Load pre-encoded Opus file (TTS now generates Opus directly)
            ...
            # Extract Opus frames (strip 2-byte length headers)
            audio_data = b''
            offset = 0
            ...
            if not audio_data:
                logging.error(f"No audio data extracted from {audio_file_path}")
                return False

            logging.info(f"📦 Loaded pre-encoded Opus: {len(audio_data)} bytes ({audio_file_path})")

            success_count = 0
            for device_id in devices:
                # Send pre-encoded Opus directly (no re-encoding needed)
                if tts_server.send_tts_audio_sync(device_id, audio_data, use_opus=True):
                    success_count += 1
                    logging.info(f"🔊 Sent TTS to {device_id} [Opus]: '{text[:30]}{'...' if len(text) > 30 else ''}'")
                else:
